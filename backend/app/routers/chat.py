...
def get_chat_history(site_id: str, db: Session = Depends(get_db), user: models.DBUser = Depends(get_current_user)):
...
def send_chat_message(req: schemas.ChatRequest, db: Session = Depends(get_db), user: models.DBUser = Depends(get_current_user)):
