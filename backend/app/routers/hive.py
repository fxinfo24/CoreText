...
def get_hive_learnings(db: Session = Depends(get_db), user: models.DBUser = Depends(get_current_user)):
    hives = db.query(models.DBHiveLearning).all()
    return hives

@router.post("/hive/transfer/{hive_id}")
def transfer_hive_strategy(hive_id: str, db: Session = Depends(get_db), user: models.DBUser = Depends(get_current_user)):
